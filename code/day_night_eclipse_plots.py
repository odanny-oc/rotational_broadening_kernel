...
orbital_phase_pre_eclipse = np.linspace(-0.1, -0.04, n_exposure)  # time/period
orbital_phase_post_eclipse = np.linspace(0.04, 0.1, n_exposure)  # time/period
...
vsys_pre = np.linspace(-200, 200, 1001)
vsys_post = np.linspace(-200, 200, 1001)

vsys_kp = np.linspace(-200, 200, 1001)
...
spectrum_pre_day = Time_Dependent_Spectrum(
    wl=wl,
    flux=flux_day,
    op=orbital_phase_pre_eclipse,
    wl_grid=wavelength_grid,
    kernel=time_dependent_broadening_kernels_pre_eclipse,
    kp=Kp,
)

spectrum_post_day = Time_Dependent_Spectrum(
    wl=wl,
    flux=flux_day,
    op=orbital_phase_post_eclipse,
    wl_grid=wavelength_grid,
    kernel=time_dependent_broadening_kernels_post_eclipse,
    kp=Kp,
)

spectrum_pre_night = Time_Dependent_Spectrum(
    wl=wl,
    flux=flux_night,
    op=orbital_phase_pre_eclipse,
    wl_grid=wavelength_grid,
    kernel=anti_kernels_pre_eclipse,
    kp=Kp,
)

spectrum_post_night = Time_Dependent_Spectrum(
    wl=wl,
    flux=flux_night,
    op=orbital_phase_post_eclipse,
    wl_grid=wavelength_grid,
    kernel=anti_kernels_post_eclipse,
    kp=Kp,
)

total_spectrum_pre = spectrum_pre_night + spectrum_pre_day

total_spectrum_post = spectrum_post_night + spectrum_post_day

CC_day_pre = Cross_Correlator(
    wl=wavelength_grid,
    flux=flux_grid_day,
    vsys=vsys_pre * 1000,
    spectrum=total_spectrum_pre,
)
...
fig.suptitle('CC Pre-Eclipse')
ax[0].set_title('Day')
ax[0].pcolormesh(vsys_pre, orbital_phase_pre_eclipse, CC_day_pre)

ax[1].set_title('Night')
ax[1].pcolormesh(vsys_pre, orbital_phase_pre_eclipse, CC_night_pre)

fig, ax = plt.subplots(2, sharex='all', sharey='all')
fig.suptitle('CC Post-Eclipse')
ax[0].set_title('Day')
ax[0].pcolormesh(vsys_post, orbital_phase_post_eclipse, CC_day_post)

ax[1].set_title('Night')
ax[1].pcolormesh(vsys_post, orbital_phase_post_eclipse, CC_night_post)

Kp_night_pre, _ = Kp_vsys_Plotter(
    op=orbital_phase_pre_eclipse, CC=CC_night_pre, vsys=vsys_kp, K=K
)
Kp_day_pre, _ = Kp_vsys_Plotter(
    op=orbital_phase_pre_eclipse, CC=CC_day_pre, vsys=vsys_kp, K=K
)

Kp_night_post, _ = Kp_vsys_Plotter(
    op=orbital_phase_post_eclipse, CC=CC_night_post, vsys=vsys_kp, K=K
)
Kp_day_post, _ = Kp_vsys_Plotter(
    op=orbital_phase_post_eclipse, CC=CC_day_post, vsys=vsys_kp, K=K
)

Kp_night = Kp_night_pre + Kp_night_post
Kp_day = Kp_day_pre + Kp_day_post

fig, ax = plt.subplots(2, sharex="all", sharey="all")
fig.suptitle('Pre-Eclipse')
ax[0].set_title("Day")
ax[1].set_title("Night")
ax[0].pcolormesh(vsys_kp, K, Kp_day_pre)
ax[1].pcolormesh(vsys_kp, K, Kp_night_pre)

ax[0].axhline(
    K[maxIndex(Kp_day_pre)[0]],
    ls="--",
    color="red",
    lw=0.5,
    label=f"Kp = {K[maxIndex(Kp_day_pre)[0]]:.2f}",
)
ax[0].axvline(
    vsys_kp[maxIndex(Kp_day_pre)[1]],
    ls="--",
    color="red",
    lw=0.5,
    label=f"vsys = {vsys_kp[maxIndex(Kp_day_pre)[1]]:.2f}",
)

ax[1].axhline(
    K[maxIndex(Kp_night_pre)[0]],
    ls="--",
    color="red",
    lw=0.5,
    label=f"Kp = {K[maxIndex(Kp_night_pre)[0]]:.2f}",
)
ax[1].axvline(
    vsys_kp[maxIndex(Kp_night_pre)[1]],
    ls="--",
    color="red",
    lw=0.5,
    label=f"vsys = {vsys_kp[maxIndex(Kp_night_pre)[1]]:.2f}",
)
...
ax[0].set_title("Day")
ax[1].set_title("Night")
ax[0].pcolormesh(vsys_kp, K, Kp_day_post)
ax[1].pcolormesh(vsys_kp, K, Kp_night_post)

ax[0].axhline(
    K[maxIndex(Kp_day_post)[0]],
    ls="--",
    color="red",
    lw=0.5,
    label=f"Kp = {K[maxIndex(Kp_day_post)[0]]:.2f}",
)
ax[0].axvline(
    vsys_kp[maxIndex(Kp_day_post)[1]],
    ls="--",
    color="red",
    lw=0.5,
    label=f"vsys = {vsys_kp[maxIndex(Kp_day_post)[1]]:.2f}",
)

ax[1].axhline(
    K[maxIndex(Kp_night_post)[0]],
    ls="--",
    color="red",
    lw=0.5,
    label=f"Kp = {K[maxIndex(Kp_night_post)[0]]:.2f}",
)
ax[1].axvline(
    vsys_kp[maxIndex(Kp_night_post)[1]],
    ls="--",
    color="red",
    lw=0.5,
    label=f"vsys = {vsys_kp[maxIndex(Kp_night_post)[1]]:.2f}",
)
